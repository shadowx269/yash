...
export const ProductCard = ({ product, onSelect }: ProductCardProps) => {
  ...
  return (
    <Card
      className="group cursor-pointer overflow-hidden hover:shadow-glow-lg transition-all duration-500 hover:-translate-y-2 rounded-2xl border-2 hover:border-primary/20 animate-fade-in"
      ...
      <CardContent className="p-0">
        <div className="relative aspect-[3/4] overflow-hidden bg-muted">
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`absolute top-3 right-3 bg-background/95 hover:bg-background backdrop-blur-sm transition-all duration-300 rounded-full shadow-md hover:scale-110 ${isWishlisted ? 'text-primary' : 'text-muted-foreground'
              }`}
          ...
          <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="flex items-center gap-1">
              {product.colors?.slice(0, 3).map((c, i) => (
                <span key={i} className="w-4 h-4 rounded-full border shadow-sm" style={{ backgroundColor: c }} />
              ))}
            </div>
            <button
              type="button"
              className="pointer-events-auto text-xs font-semibold px-3 py-1.5 rounded-full bg-background/90 border shadow-sm hover:shadow-md"
        ...
        <div className="p-4 space-y-2">
          <h3 className="font-heading font-semibold text-sm line-clamp-2 mb-1 group-hover:text-primary transition-colors duration-300">{product.name}</h3>
          {product.fabric && (
            <p className="text-xs text-muted-foreground font-medium">{product.fabric}</p>
          )}
